...
class APIHTTPConnector(object):
    ...
    def fire_request(self, converter_instance):
        ...
        """
        This method makes a request to the Zoho Rest API
        ...
        if self.request_method == Constants.REQUEST_METHOD_GET:
            ...
            if self.file:
                response = requests.post(url=self.url, files=data, params=self.parameters, headers=self.headers, allow_redirects=False, data={},
                                         proxies=proxies)
            else:
